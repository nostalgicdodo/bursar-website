...
$dependencies = [
	...
	'emojis' => 'Emojis',
	'rss' => 'RSS',
	// 'jqueryMigrate' => 'jQueryMigrate',
	// 'jqueryAlias' => 'jQueryAlias',
];
...
class jQueryMigrate {
	public static function remove () {
		add_action( 'wp_default_scripts', function ( $scripts ) {
			...
			$jQueryScriptHandle = $scripts->registered[ 'jquery' ] ?? null;
			if ( empty( $jQueryScriptHandle ) )
				return;

			$jQueryScriptHandle->deps = array_diff(
				$jQueryScriptHandle->deps,
				[ 'jquery-migrate' ]
			);
		} );
	}
...
class jQueryAlias {
	public static function add () {
		add_action( 'wp_enqueue_scripts', function () {
			wp_add_inline_script( 'jquery', 'window.$ = jQuery;' );
		} );
	}
}
